...
const TourCardDesc = ({tour}) => {
    return (
        <Box sx={card_desc}>
            ...
            <Box>
                <Box sx={card_price}>
                    <Typography sx={{fontWeight: '700', fontSize: '24px'}}>{tour.price_KGZ}</Typography>
            ...
            <Typography sx={{fontWeight: '400', fontSize: '20px'}}>{tour.difficulty_level}</Typography>
            <Box sx={{display: 'flex', gap: '10px'}}>
                <DateFormat date={tour.start_dates[0]}/>

                {
                    tour.start_dates.length > 1 ?
                        <Box sx={card_dates}>
                            + {tour.start_dates.length} даты
                        </Box> :
                        <Box sx={card_dates}>
                            + 1 дата
                        </Box>
